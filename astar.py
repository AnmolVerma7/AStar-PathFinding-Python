...
def algorithm(draw, grid, start, end):
    ...
    # What node did current node come from (previous node) 
    came_from = {}

    # Keeps track of the current shortest distance to get from start node to this node
    ...
    # Estimate of start node to end node
    f_score[start] = h(start.get_pos(), end.get_pos()) 

    # See if there is something in open set
    open_set_hash = {start}

    # While open set is not empty
    while not open_set.empty():

        # Make sure user is not quitting application
        for event in pygame.event.get():

            # If user is quitting, quit the game
            if event.type == pygame.QUIT:
                pygame.quit()

        # Finds lowest element from Priority Queue
        current = open_set.get()[2]
        open_set_hash.remove(current)

        # If at the end, path is found
        if current == end:
        ...
        for neighbor in current.neighbors: 
            # Calculate temp G-Score
             # The cost of going from start to here
            temp_g_score = g_score[current] + 1

            # If temp G-Score is less than what the G-Score is on the table
            if temp_g_score < g_score[neighbor]:
                
                # Update the path and score and store it
                came_from[neighbor] = current
                g_score[neighbor] = temp_g_score
                f_score[neighbor] = temp_g_score + h(neighbor.get_pos(), end.get_pos())

                # If open set hash does not contain scores, add them 
        ...
        # If current is not the start node, make it red and close it off
        if current != start:
            current.make_closed()
